...
function parseLessonTextBlocks(lessonText: string): ParsedLessonBlocks {
  const lessonBody = String(lessonText || "").trim();
  if (!lessonBody) return { passage: "", questions: [] };
  ...
  const passageSectionMatch = lessonBody.match(/PASSAGE:\s*([\s\S]*?)(?:QUESTION|$)/i);
  const passage = passageSectionMatch ? passageSectionMatch[1].trim() : "";

  const questions = lessonBody
    .split(/QUESTION/i)
    .slice(1)
    .map((block) => ({
      raw: block.trim(),
