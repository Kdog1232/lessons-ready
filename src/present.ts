...
function parseLessonTextBlocks(lessonText: string): ParsedLessonBlocks {
  const sourceText = String(lessonText || "").trim();
  if (!sourceText) return { passage: "", questions: [] };
  ...
  const passageSectionMatch = sourceText.match(/PASSAGE:\s*([\s\S]*?)(?:QUESTION|$)/i);
  const passage = passageSectionMatch ? passageSectionMatch[1].trim() : "";

  const questions = sourceText
    .split(/QUESTION/i)
    .slice(1)
    .map((block) => ({
      raw: block.trim(),
