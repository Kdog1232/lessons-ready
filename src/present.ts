...
function renderSlide() {
  ...
  if (slide.type === "splash") {
    ...
    if (slide.stageType === "verb_definition") {
      container.innerHTML = `<div class="slide slide-content ${layoutClass} slide--headline${extraClass}">${stage}${coachingTag}<div class="vocab-box interactive fade-in" style="animation-delay:0.05s"><h2>${escHtml(cleanHeading(limitText(cleanText(String(slide.heading || "")), 90)) || "Skill Focus")}</h2><p>${escHtml(limitText(cleanText(String(slide.subtext || "")), 240))}</p></div><div class="vocab-box interactive fade-in" style="animation-delay:0.15s"><h2>Student-Friendly Move</h2><p>Say the verb in your own words, then name the evidence you need.</p></div>${coachLine}${alignment}</div>`;
    } else {
      container.innerHTML = `<div class="slide slide-content ${layoutClass} slide--headline${extraClass}">${stage}${coachingTag}<div class="sectionTag fade-in" style="animation-delay:0.05s">${escHtml(limitText(cleanText(String(slide.section || "")), 40))}</div><h1 class="fade-in" style="animation-delay:0.05s">${escHtml(cleanHeading(limitText(cleanText(String(slide.heading || "")), 90)) || "Lesson Focus")}</h1><p class="fade-in" style="animation-delay:0.15s">${escHtml(limitText(cleanText(String(slide.subtext || "")), 240))}</p>${coachLine}${alignment}</div>`;
    }
    ...
    container.innerHTML = renderMultipleChoice(slide, extraClass, layoutClass, stage, coachingTag, coachLine, alignment);
    ...
    if (slide.stageType === "compare_defend") {
      container.innerHTML = `
        <div class="slide slide-content ${layoutClass} slide--discussion${extraClass}">
          ...
          <div class="debate-box interactive fade-in" style="animation-delay:0.15s"><h3>Option A</h3><p>${promptClean}</p></div>
          <div class="debate-box interactive fade-in" style="animation-delay:0.25s"><h3>Option B</h3><p>Defend the stronger answer with evidence.</p></div>
          ${stem}
          ${coachLine}${alignment}
        </div>
      `;
    ...
    container.innerHTML = `<div class="slide slide-content ${layoutClass}${extraClass}">${stage}${coachingTag}<h2 class="fade-in" style="animation-delay:0.05s">${escHtml(cleanHeading(limitText(cleanText(String(slide.heading || "Slide")), 80)) || "Slide")}</h2>${coachLine}${alignment}</div>`;
  }
