...
function getSavedLesson(): (LessonRow & { slide_definitions?: any[]; slides?: any[]; questions?: any[]; id?: string }) | null {
  const storedLessonRaw = localStorage.getItem("lr_current_lesson");
  if (!storedLessonRaw) return null;

  try {
    return JSON.parse(storedLessonRaw) as LessonRow & { slide_definitions?: any[]; slides?: any[]; questions?: any[]; id?: string };
  } catch (error) {
...
async function boot() {
  ...
  const storedLesson = localStorage.getItem("lr_current_lesson")
